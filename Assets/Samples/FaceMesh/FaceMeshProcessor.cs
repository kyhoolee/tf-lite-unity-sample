...
public sealed class FaceMeshProcessor : MonoBehaviour
{
    ...
    [SerializeField]
    private bool useLandmarkToDetection = false;
    ...
    public FaceMesh.Result getFaceMeshResult() => meshResult;
    ...
    private void DrawResults(FaceDetect.Result detection, FaceMesh.Result face)
    {
        ...
        if (face != null)
        {
            ...
            // Draw mouth 
            int[] mouths = {0, 291}; //, 61, 17};
            
            for(int i = 0 ; i < mouths.Length ; i += 2) {
                draw.color = Color.red;
                Vector3 kp1 = face.keypoints[mouths[i]];
                kp1.y = 1f - kp1.y;
                Vector3 p1 = MathTF.Lerp(min, max, kp1);
                p1.z = face.keypoints[mouths[i]].z * zScale;

                draw.Point(p1, 0.2f);
                
                Vector3 kp2 = face.keypoints[mouths[i+1]];
                kp2.y = 1f - kp2.y;
                Vector3 p2 = MathTF.Lerp(min, max, kp2);
                p2.z = face.keypoints[mouths[i+1]].z * zScale;
                draw.Point(p2, 0.2f);

                draw.Apply();

                draw.color = Color.gray;
                draw.Line(p1, p2, 0.1f);
                draw.Apply();
            }
